...
// ─── Bot config snapshot passed from the caller ───────────────────────────────
export interface BotConfigSnapshot {
  version: number;
  updatedAt: string | null;
  updatedBy: string | null;
  /** Whether the PUT /api/bot-config health check passed */
  healthy: boolean;
  /** How many top-level keys the config document has */
  fieldCount?: number;
}
...
export async function sendHourlyBackupReport(
  ...
  if (botConfig) {
    const statusColor  = botConfig.healthy ? '#4ade80'  : '#f87171';
    const statusText   = botConfig.healthy ? 'HEALTHY'  : 'UNREACHABLE';
    const statusBg     = botConfig.healthy ? '#0d1f0d'  : '#1f0d0d';
    const statusBorder = botConfig.healthy
      ? 'rgba(74,222,128,0.2)'
    ...
    botConfigSection = `
      <!-- Bot Config Section -->
      <div style="margin-top:12px;border:1px solid ${statusBorder};background:${statusBg};padding:20px 24px;">
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
          <div>
            <div style="display:flex;align-items:center;gap:10px;margin-bottom:6px;">
              ...
              <span style="font-family:monospace;font-size:11px;font-weight:700;color:${statusColor};
                border:1px solid ${statusColor};padding:2px 8px;opacity:0.85;">
                ● ${statusText}
              </span>
            </div>
            ${updatedLine}
...
<body style="margin:0;padding:0;background:#0a0a0a;color:#fff;font-family:system-ui,sans-serif;">
  ...
  const subject = logs.length === 0
    ? `[Hackoverflow] No backups recorded — Bot Config ${botConfig ? `v${botConfig.version}` : ''} ${formatTime(new Date())}`
    : `[Hackoverflow] ${succeeded.length}/${logs.length} backups succeeded — Bot Config ${botConfig ? `v${botConfig.version} ${botConfig.healthy ? '✓' : '✗'}` : ''} — ${formatTime(new Date())}`;
